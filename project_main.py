...
ax_g.set_title('Top Chemical Classes')
ax_g.invert_yaxis()
...
def property_boxplot_top_classes(df, prop, class_col=CLASS_COL, n_classes=8, outprefix=None):
    ...
    # Define a color palette (one color per class)
    palette = sns.color_palette("Set2", n_classes)  # you can choose other palettes
    
    # Create HORIZONTAL boxplot with thin boxes
    sns.boxplot(
        y=class_col,
        x=prop,
        data=data,
